...
=<
  ...
  ++  on-poke
    ...
    ?+  mark  (on-poke:def mark vase)
      ...
      :: %invite-action    (on-invite-action:core !<(action:invite-store vase))
    ==
    [cards this]
    --
  ++  on-peek
    ...
    ?+    path  (on-peek:def path)
      ...
      ``noun+!>((view:enjs:lib [%space [ship space-pth] space]))
      :: ::
...
++  on-spaces-action
  ...
  ++  handle-add
    ...
    ::  TODO forward to passport agent
    :_  state
...
++  has-auth
  ...
  ::  TODO scry passports
  =/  space   .^(space:spaces %gx /(scot %p our.bowl)/passports/(scot %da now.bowl)/passports/[path]/noun)
  ::  
