...
const request =
  ...
  function (callback: Callback): RequestAnimationFrame {
    return (win as any).setTimeout(callback, 1000 / 60);
  };
...
const cancel =
  ...
  function (id: number) {
    (win as any).clearTimeout(id);
  };

export const raf: RequestAnimationFrame = request as any;
export const caf: CancelAnimationFrame = cancel as any;
