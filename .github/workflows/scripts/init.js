...
module.exports = ({ github, context }, pkgfile) => {
  ...
  if (context.pull_request.title) {
    console.log(
      `init.js: PR title = '${context.pull_request.title}'. testing if matches version format...`
    );
    const matches = context.pull_request.title.match(
      /(release|staging)-(v|)(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)(?:-((?:0|[1-9]\d*|\d*[a-zA-Z-][0-9a-zA-Z-]*)(?:\.(?:0|[1-9]\d*|\d*[a-zA-Z-][0-9a-zA-Z-]*))*))?(?:\+([0-9a-zA-Z-]+(?:\.[0-9a-zA-Z-]+)*))?$/
    );
    if (matches) {
      console.log(
        `init.js: '${context.pull_request.title}' matches version format. using as version string.`
      );
      // set version from PR title "as-is". do not auto increment
      incrementVersion = false;
      ci.buildVersion = context.pull_request.title;
