...
export default class EncryptedStore <T> {
  ...
  encryptData(data: any): string {
    return CryptoJS.AES.encrypt(JSON.stringify(data), safeStorage.decryptString(this.secretKey)).toString();
  }

  decryptData(ciphertext: string) {
    let bytes = CryptoJS.AES.decrypt(ciphertext, safeStorage.decryptString(this.secretKey));
