...
export class WalletsDB extends AbstractDataAccess<Wallet> {
  ...
  protected mapRow(row: any): Wallet {
    return {
      space: row.space,
      patp: row.patp,
  ...
  public insertAll(spacesMembers: { [key: string]: Wallet[] }) {
  ...
  public createMember(values: Partial<Wallet>): Wallet {
  ...
  public getMember(path: string, patp: string): Wallet | null {
  ...
  public create(values: Partial<Wallet>): Wallet {
    if (values.roles) values.roles = JSON.stringify(values.roles);
    const columns = Object.keys(values).join(', ');
    const placeholders = Object.keys(values)
      .map(() => '?')
  ...
  public update(path: string, values: Partial<Wallet>): Wallet {
    const setClause = Object.keys(values)
      .map((key) => `${key} = ?`)
      .join(', ');
...
export const spacesMembersDBPreload = WalletsDB.preload(new WalletsDB(true));
