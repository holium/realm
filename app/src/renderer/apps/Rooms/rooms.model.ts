...
export const RoomsModel = types
  ...
  .actions((self) => ({
    kickShip(string: string) {
      self.present.remove(string);
      self.whitelist.remove(string);
    },
  }));
...
export const RoomsAppState = types
  .model('RoomsAppState', {
    currentRoomId: types.maybeNull(types.string),
    ...
    ourstring: types.maybe(types.string),
  ...
  .views((self) => ({
    ...
    isCreator(string: string, roomId: string) {
      return self.knownRooms.get(roomId)?.creator === string;
    },
  ...
  .actions((self) => ({
    ...
    handleRoomUpdate(room: RoomsModelType, diff: any) {
      ...
      if (!knownRoom && room.creator === self.ourstring) {
    ...
    removeSelf(roomId: string, string: string) {
      self.knownRooms.get(roomId)?.present.remove(string);
    },
    leaveRoom() {
      // if (self.liveRoom) {
    ...
    kickRoom(string: string, roomId: string) {
      self.knownRooms.get(roomId)?.kickShip(string);
    ...
    appendOurChat(our: string, contents: string) {
      const time = Date.now();
    ...
    handleInboundChat(from: string, contents: string) {
      const time = Date.now();
      const chat: ChatModelType = {
        author: from,
        index: time,
