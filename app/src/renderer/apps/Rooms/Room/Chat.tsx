...
const RoomChatPresenter = () => {
  ...
  const { roomsApp, getTrayAppHeight } = useTrayApps();
  ...
  const handleChat = useCallback(
    (evt: any) => {
      ...
      if (!roomsApp.liveRoomId) return;
      const innerText = chatInputRef.current.value;
      if (innerText === '') return;
      roomsStore.sendChat(roomsApp.liveRoomId, innerText);
      text.actions.onChange('');
    },
    [roomsApp.liveRoomId, text.actions]
  );
