...
const WalletInfo: FC<{ hideWalletHero: boolean}> = (props: { hideWalletHero: boolean }) => {
  return (
    <Flex p={2} width="100%"
      ...
      <Flex width="100%" justifyContent="space-between">
        ...
        <Flex>
          ...
          <Box onClick={() => setQROpen(!QROpen)}>
            <Icons ml={2} name="QRCode" height="20px" color={QROpen ? theme.colors.brand.primary : theme.colors.text.tertiary} />
          </Box>
        </Flex>
      </Flex>
...
const AmountInput = (props: { max: number, setValid: (valid: boolean, amount?: number) => void }) => {
  ...
  const onChange = (e: any) => {
    ...
    if (!inCrypto && isDecimal && decimalPlaces > 2)
      return
      ...
      <FlexHider width="100%" justifyContent="space-evenly" alignItems="center">
        ...
        <Flex px={1} py={1} width="200px" height="40px" justifyContent="space-between" borderRadius="7px" background={panelBackground} border={`solid 1px ${amountError ? theme.colors.text.error : theme.colors.ui.borderColor}`}>
          <Flex pl="10px" flexDirection="column" justifyContent="center" alignItems="flex-start">
            {inCrypto
              ? <Input type="number" placeholder="0.00000000" value={amount || ''} onChange={onChange} />
              :(
                <Flex>
                  <Text pt="2px" fontSize="12px">$</Text>
                  <Input type="number" placeholder="0.00" value={amount || ''} onChange={onChange} />
                </Flex>
              )}
            <Box hidden={!amount}>
              <Text fontSize="11px" color={theme.colors.text.disabled}>
                {inCrypto
                  ...
                  : `${usdToEth(Number(amount))} ${abbrMap[walletApp.network as 'bitcoin' | 'ethereum']}`}
              </Text>
            </Box>
          </Flex>
          <Flex p="4px" justifyContent="center" alignItems="center" background={lighten(.02, desktop.theme.windowColor)} border={`solid 1px ${theme.colors.ui.borderColor}`} borderRadius="5px" onClick={toggleInCrypto}>
...
const RecipientInput = (props: { setValid: (valid: boolean, recipient: { address?: string, patp?: string }) => void }) => {
  ...
  const RecipientIcon = (props: { icon: string }) => {
    ...
    if (props.icon === 'sigil')
      return  <Sigil color={['black', 'white']} simple={true} size={24} patp={valueCache!} />
  ...
  return (
    <Flex width="100%" justifyContent="space-evenly" alignItems="center">
      ...
      <Flex px={1} py={1} width="240px" height="45px" borderRadius="7px" alignItems="center" background={panelBackground} border={`solid 1px ${recipientError ? theme.colors.text.error : theme.colors.ui.borderColor}`}>
        ...
        <Input width="100%" placeholder="@p or recipient’s address" spellCheck="false" value={recipient} onChange={onChange} />
      </Flex>
    </Flex>
  )
}

const TransactionPane = (props: { max: number, setHideWalletHero: any, close: any }) => {
      ...
      ? (
        <Flex mt={7} flexDirection="column">
          ...
          <Flex mt={7} justifyContent="space-between">
            ...
            <Button px={4} disabled={!recipientValid || !amountValid} onClick={next}>Next</Button>
          </Flex>
        </Flex>
      )
      : (
...
const SendTransaction: FC<SendTransactionProps> = (props: SendTransactionProps) => {
  const Seperator = () => (
    <Flex mt={6} position="relative" width="100%" justifyContent="center">
      <Box position="absolute" width="300px" height="1px" left="-10px" background="#EEEEEE" />
      <Flex position="absolute" bottom="-12px" height="25px" width="80px" justifyContent="center" alignItems="center" borderRadius="50px" background="#EAF3FF">
  ...
  return (
    <Box width="100%" hidden={props.hidden}>
      <Seperator />
      <TransactionPane setHideWalletHero={props.setHideWalletHero} close={props.close} max={4} />
    </Box>
  )
}
  ...
  return (
    <Flex width="100%" height="100%" flexDirection="column" p={3}>
      ...
      <Flex p={3} width="100%" flexDirection="column" background={lighten(.02, desktop.theme.windowColor)} boxShadow="0px 0px 9px rgba(0, 0, 0, 0.12)" borderRadius="16px">
        ...
        <SendReceiveButtons />
        <SendTransaction hidden={!sendTrans} setHideWalletHero={setHideWalletHero} close={() => {
          setSendTrans(false);
