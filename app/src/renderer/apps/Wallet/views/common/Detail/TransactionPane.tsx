...
border: 0;
color: ${(props) => props.mode === 'light' ? '#333333' : '#ffffff'} !important;
outline: none;
...
export const TransactionPane: FC<TransactionPaneProps> = observer((props: TransactionPaneProps) => {
  const { walletApp } = useTrayApps();
    ...
    { screen === 'initial'
      ...
      : (
        <Flex mt={7} flexDirection="column">
          ...
          <Text mt={1} color={theme.colors.text.disabled}>
            ${ethToUsd(transactionAmount)} USD
          </Text>
          <Flex mt={7} width="100%" flexDirection="column" justifyContent="space-evenly" alignItems="center">
            <Flex width="100%" justifyContent="space-between">
              <Text variant="body" color={theme.colors.text.secondary}>TO</Text>
              <Flex justifyContent="center">
                <Flex mr={2}>
                  {transactionRecipient.address
                    ? <Icons name="Spy" size="24px" color={theme.colors.text.secondary} />
                    : <Sigil color={desktop.theme.mode === 'light' ? ['black', 'white'] : ['white', 'black']} simple={true} size={24} patp={transactionRecipient.patp!} />
                  }
                </Flex>
                <Flex flexDirection="column" justifyContent="center">
                  {transactionRecipient.address
                    ? <Text variant="body">{shortened(transactionRecipient.address)}</Text>
                    : <>
                      <Text variant="body">{transactionRecipient.patp}</Text>
                      <Text variant="body" color={theme.colors.text.tertiary}>{shortened(`0xE96E2181F6166A37EA4C04F6E6E2bD672D72Acc1`)}</Text>
                    </>
            ...
            <Flex mt={5} width="100%" justifyContent="space-between">
              <Text variant="body" color={theme.colors.text.secondary}>NETWORK FEE</Text>
              <Flex flexDirection="column">
                <Text variant="body">0.0005 ETH</Text>
                <Text fontSize={1} color={theme.colors.text.secondary}>≈ {ethToUsd(0.0005)} USD</Text>
              </Flex>
            </Flex>
            <Flex mt={5} width="100%" justifyContent="space-between">
              <Text variant="body" color={theme.colors.text.secondary}>TOTAL</Text>
              <Flex flexDirection="column">
                <Text variant="body">3.4505 ETH</Text>
                <Text fontSize={1} color={theme.colors.text.secondary}>≈ {ethToUsd(3.4505)} USD</Text>
              </Flex>
            </Flex>
          </Flex>
          <Flex mt={7} justifyContent="space-between">
            <Button variant="transparent" onClick={() => prev()}>Reject</Button>
            <Button px={2} onClick={sendTransaction}>Confirm</Button>
          </Flex>
        </Flex>
      )}
    </>
